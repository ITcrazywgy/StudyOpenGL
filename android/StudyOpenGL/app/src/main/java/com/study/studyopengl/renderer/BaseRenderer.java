...
public abstract class BaseRenderer implements GLSurfaceView.Renderer {
    ...
    private final float[] mvpMatrix = new float[16];// mpvMatrix = projectMatrix * viewMatrix * modelMatrix
    ...
    //透视矩阵
    void frustumM(float left, float right, float bottom, float top, float near, float far) {
        Matrix.frustumM(projectMatrix, 0, left, right, bottom, top, near, far);
    }

    //正交矩阵
    void orthoM(float left, float right, float bottom, float top, float near, float far) {
        //透视矩阵
